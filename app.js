...
function renderResults() {
  const output = results.map(item => `<p class="search_item"><div class="search_link"><a href="ronbronson/status/${item.doc.id_str}">link</a></div> <div class="search_text">${item.doc.full_text}</div><div class="search_time">${new Date(item.doc.created_at).toLocaleString()}</div><hr class="search_divider" /></p>`.replace(/\.\.\/\.\.\/tweets_media\//g,'ronbronson/tweets_media/'));
